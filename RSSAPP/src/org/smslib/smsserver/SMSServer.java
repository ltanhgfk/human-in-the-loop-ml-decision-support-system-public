...
public class SMSServer
{
	...
	public SMSServer()
	{
		...
		this.inboundNotification = new InboundNotification();//tin nhan den he thong
		this.outboundNotification = new OutboundNotification();//tin nhan gui tu he thong
	...
	private void loadConfiguration() throws Exception
	{
		...
		try
		{
			...
			else f = new FileInputStream("./smsconf/SMSServer.conf");
	...
	private void process() throws Exception
	{
		...
		while (!this.shutdown)
		{
			...
			sleeptime = sleeptime.getConfigBycode("sleep_time_get_sms");
			Thread.sleep(sleeptime.getvalueint()*1000);
			//Thread.sleep(1000);// can set bien cau hinh cho nay
		}
	}
	...
	public void stopInterfaces() throws Exception // cu la private => doi lai de truy cap tu thread
	{
		for (Interface<? extends Object> inf : getInfList())
			inf.stop();
	}
	...
	void readMessages()
	{
		...
		try
		{
			...
			if (msgList.size() > 0)
			{
				...
				// cai dat lai thanh yes neu muon xoa tin nhan trong modem sau khi ghi vao csdl - trong file SMSServer.conf
			}
		}
		catch (Exception e)
		{
